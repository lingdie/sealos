...
func (r *DevboxReconciler) syncNetwork(ctx context.Context, devbox *devboxv1alpha2.Devbox, recLabels map[string]string) error {
	...
	pipeline := []func(context.Context, *devboxv1alpha2.Devbox, map[string]string) error{
		r.syncHeadlessService,
		r.syncNodeportService,
		r.syncTailnet,
		r.syncSSHGate,
	}
...
// sync headless service, create a headless service for the devbox.
func (r *DevboxReconciler) syncHeadlessService(ctx context.Context, devbox *devboxv1alpha2.Devbox, recLabels map[string]string) error {
	service := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      fmt.Sprintf("%s-headless", devbox.Name),
			Namespace: devbox.Namespace,
			Labels:    recLabels,
		},
	}
...
func (r *DevboxReconciler) syncNodeportService(ctx context.Context, devbox *devboxv1alpha2.Devbox, recLabels map[string]string) error {
	service := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
